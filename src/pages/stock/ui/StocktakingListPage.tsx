...
const StocktakingListPage: Component = () => {
    ...
    const reload = async () => {
        ...
        stocks.sort((a, b) => b.stocktakingDate.getTime() - a.stocktakingDate.getTime());
		...
		<article class="size-full flex flex-col gap-10 p-10">
			<section class="flex items-center justify-between">
				<div class="flex items-center gap-10">
					<DateInput label="検索（開始日）" value={periodStart()} onChange={setPeriodStart}/>
					<DateInput label="検索（終了日）" value={periodEnd()} onChange={setPeriodEnd}/>
				</div>
			</section>
				...
				>
					<ul class="flex flex-col gap-5">
						<For each={stocktakings()}>
							{(stocktaking) => (
								<li class="">
									<details class="collapse collapse-arrow bg-base-200" name={`stocktaking-${stocktaking.id}`}>
